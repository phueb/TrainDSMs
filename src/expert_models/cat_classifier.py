...
def hc_data_path(self):
    file_name = ''.join(
        '{}{}'.format(key, val) for key, val in sorted(AppConfigs.HC_PARAMS_DICT.items()))  # TODO test
    file_name = file_name.replace('.', '_')
    hc_data_path = GlobalConfigs.RUNS_DIR / self.model_name / 'Classifier' / file_name + '.npz'
    return hc_data_path
    
def make_classifier_data(self,  # TODO improve this whole fn, it is old
                         num_acts_samples,
                         shuffle_cats,
                         rep_type,
                         random_x):
    ...
    if rep_type == 'proto':
        ...
        for n, probe_id in enumerate(multi_probe_acts_df.index.tolist()):
            probe = self.hub.probe_store.types[probe_id]
    ...
    if random_x:
        print('Creating a random input matrix')
        x_data = np.random.rand(*x_data.shape)
    return x_data, y_data

def run_classifier(self,
                   hc_params_dict=AppConfigs.HC_PARAMS_DICT):
    print('Classifying hidden representations...')
    num_epochs = hc_params_dict['num_epochs']
    mb_size = hc_params_dict['mb_size']
    num_acts_samples = hc_params_dict['num_acts_samples']
    num_steps_to_eval = hc_params_dict['num_steps_to_eval']
    learning_rate = hc_params_dict['learning_rate']
    num_hiddens = hc_params_dict['num_hiddens']
    rep_type = hc_params_dict['rep_type']
    shuffle_cats = hc_params_dict['shuffle_cats']
    random_x = hc_params_dict['random_x']
    for k, v in AppConfigs.HC_PARAMS_DICT.items():
        print(k, v)
    # load_data
    x_data, y_data = self.make_classifier_data(num_acts_samples, shuffle_cats, rep_type, random_x)
    ...
    num_evals = max_steps // num_steps_to_eval
    ...
    with tf.Graph().as_default() and tf.device(AppConfigs.APP_DEVICE):
        # placeholders
        x = tf.placeholder(tf.float32, shape=(None, input_dim))
        y = tf.placeholder(tf.int32, shape=(None))
        # hidden ops
        with tf.name_scope('hidden'):
            weights = tf.Variable(tf.truncated_normal([input_dim, num_hiddens],
                                                      stddev=1.0 / sqrt(input_dim)), name='weights')
            biases = tf.Variable(tf.zeros([num_hiddens]))
