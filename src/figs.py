...
def make_classifier_figs(train_trajs, test_trajs, x_mat, cm):
    ...
    def make_classifier_accuracy_fig():
        ...
        """
        fig, ax = plt.subplots(1, figsize=(config.Fig.width, 3), dpi=config.Fig.dpi)
        ax.set_ylim([0, 110])
        ax.set_xlabel('Training steps', fontsize=config.Fig.AXLABEL_FONT_SIZE)
        ylabel = 'Accuracy'
        ...
        ax.set_ylabel(ylabel, fontsize=config.Fig.AXLABEL_FONT_SIZE)
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
        ax.tick_params(axis='both', which='both', top='off', right='off')
        ax.yaxis.grid(True)
        # plot
        ax.plot(train_hca_traj, '-', linewidth=config.Fig.LINEWIDTH, label='train')
        ax.plot(test_hca_traj, '-', linewidth=config.Fig.LINEWIDTH, label='test')
        ax.legend(loc='best')
        plt.tight_layout()
        return fig
    ...
    def make_classifier_accuracy_breakdown_fig():
        ...
        fig, axarr = plt.subplots(model.hub.probe_store.num_cats, 1,
                                  figsize=(config.Fig.width, 3 * model.hub.probe_store.num_cats),
                                  dpi=config.Fig.dpi)
        for ax, train_hca_cat_traj, test_hca_cat_traj, cat, x in zip(
            ...
            ax.yaxis.grid(True)
            # plot
            ax.plot(x, train_hca_cat_traj, '-', linewidth=config.Fig.LINEWIDTH, label='{} (train)'.format(cat))
            ax.plot(x, test_hca_cat_traj, '-', linewidth=config.Fig.LINEWIDTH, label='{} (test)'.format(cat))
            ax.legend()
        plt.tight_layout()
        return fig
    ...
    def make_classifier_conf_mat_fig():
        ...
        """
        fig, ax = plt.subplots(1, 1, figsize=(config.Fig.width, config.Fig.width),
                               dpi=config.Fig.dpi)
        # plot
