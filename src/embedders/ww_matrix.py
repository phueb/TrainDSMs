...
class WWEmbedder(EmbedderBase):
    ...
    def update_matrix(self, mat, ids):
        windows = itertoolz.sliding_window(config.WW.window_size, ids)
        for w in windows:
            for t1_id, t2_id, dist in zip([w[0]] * (config.WW.window_size - 1), w[1:], range(1, config.WW.window_size)):
                if config.WW.window_weight == "linear":
                    mat[t1_id, t2_id] += config.WW.window_size - dist
