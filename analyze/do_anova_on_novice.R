...
with(dat, interaction.plot(task, embedder, score,
  ylim = c(0.5, 1), lwd = 5,
  ylab = "mean of score", xlab = "task", trace.label = "embedder"))

# ANOVA
dat.aov <- aov(score ~ embedder * task + Error(job_name), data = dat)
summary(dat.aov)
